...
public class FileExplorer extends LireEditorWindow{
	...
	public void construct() {
		...
		scrollContent.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
				if(button == Buttons.RIGHT) {
					...
					if(Editor.getInstance().getCurrentProject() != null && getSelectedFolder() != null) {
		...
		treeExplorer.addListener(new ClickListener() {
			public void clicked(InputEvent event, float x, float y) {
				...
				if(n != null) {
					setSelectedFolder(((FileLabel) n.getActor()).getHandle());
					refreshContents();
				}
			}
		});
	...
	public void refreshContents() {
		...
		if(getSelectedFolder() != null) {
			int cont = 0;
			for(final FileHandle fh : getSelectedFolder().list()) {
	...
	public void setSelectedFolder(FileHandle selectedFolder) {
