...
public class Cloner {
	...
	static void cloneDir() {
		try {
			...
			System.out.println("Updating project root");
			...
			replaceInFile(file, "DefaultProject", projectName);
			...
			//Open in eclipse
			System.out.println("Opening in eclipse");
			openEclipse(workspaceFolder);
	...
	static void openEclipse(File workspaceFolder) {
		try {
			
			File root = new File(System.getProperty("user.dir")).getParentFile();
			String command = "py \"" + root.getAbsolutePath() + "\\loadProject.py\" \"" + workspaceFolder.getAbsolutePath() + "\" \"" + root.getAbsolutePath() + "\"";
